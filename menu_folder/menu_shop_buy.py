...
class Menu_shop_buy(pygame.sprite.Sprite):
  def __init__(self, eventList, pc_inventory):
    pygame.sprite.Sprite.__init__(self)
    self.menu_name='Menu_shop_buy'
    ...
    self.menu_op_name=[]
    self.menu_op=['Menu_shop_buy', 'Menu_shop_buy', 'Menu_shop_buy']
  ...
  def update(self, menuManager):
    ...
    for event in self.eventList:
      print(self.show_start, self.show_end, self.choice_count)
      if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_DOWN:
          ...
          self.buy()
        print("on_screen_cursor", self.on_screen_cursor)
  ...
  def buy(self):
    if self.pc_inventory.have_more_money_than(self.pc_inventory.inventory[self.menu_op_name[self.inner_cursor]].buying_price):
      self.pc_inventory.decrease_money(self.pc_inventory.item_dic[self.menu_op_name[self.inner_cursor]].buying_price)
      self.pc_inventory.add(self.menu_op_name[self.inner_cursor])
  ...
  def draw(self, screen):
    ...
    for j in range(self.show_start, self.show_end):
      ...
      self.myText=self.myFont.render(str(self.item_group_list[j].buying_price), True, (255,255,255))
