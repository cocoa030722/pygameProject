...
class Ingame_menu(pygame.sprite.Sprite):
  def __init__(self, pc_inventory):
    ...
    self.rect=ingame_menu_asset.help_menu_rect
    self.menu_tile_list=[]
    self.item_group=pygame.sprite.Group()
    
    for i in range(0, self.choice_count):
      self.menu_tile_list.append(ingame_menu_tile.Tile(i, ((vari.SCREEN_WIDTH//9)*i), ingame_menu_asset.small_menu_rect.y))

      
    idx=0
    for i in pc_inventory.inventory_group.sprites():
      if idx<self.choice_count and pc_inventory.item_count[i.name]>0:
        print(i.name)
    ...
    self.picked_item_info='pick item: '+str(self.picked_item.name)
    ...
    self.picked_item_info_rect.x=ingame_menu_asset.help_menu_rect.x+64
    self.picked_item_info_rect.y=ingame_menu_asset.help_menu_rect.y+vari.SCREEN_WIDTH//9
    
  def update(self, eventList, ingameMenuManager, pc_inventory):
    ...
    for i in pc_inventory.inventory_group.sprites():
      if (i not in self.item_group) and pc_inventory.item_count[i.name]>0:
        #i.add(self.item_group)
        ...
        for j in self.menu_tile_list:
          if j.include_item.name=='empty':
            j.add(i.name, pc_inventory)
            break
            
    for i in range(len(self.menu_tile_list)):
      self.menu_tile_list[i].update(self.menu_tile_list[i].include_item.name, pc_inventory)
  ...
  def draw(self, screen):
    screen.fill((204, 204, 204), ingame_menu_asset.help_menu_rect)
